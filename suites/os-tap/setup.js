...
async function globalSetup() {
		...
		const worker = this.require('helpers/worker');
		const balena = this.require("helpers/sdk");
		// The balenaOS class contains information on the OS image to be flashed, and methods to configure it
		const os = this.require('helpers/balenaos');
		const utils = this.require('helpers/utils');
		...
		// fix path, make configurable
		const deviceTypeContract = require(`../../core/contracts/contracts/hw.device-type/${config.deviceType}/contract.json`);

		const context ={
			sshKeyPath: join(homedir(), 'id'),
			sshKeyLabel: id,
			uuid: uid(),
			deviceTypeContract: deviceTypeContract,
			config: config
		}
		...
		if (config.networkWired === true) {
			networkConfig.wired = {
				nat: true,
			};
		} else if(deviceTypeContract.data.connectivity.wifi === false){
		...
		if (config.networkWireless === true) {
			networkConfig.wireless = {
				ssid: this.suite.options.id,
				psk: `${this.suite.options.id}_psk`,
				nat: true,
			};
		} else if(deviceTypeContract.data.connectivity.wifi === true){
		...
		const keys = await utils.createSSHKey(this.sshKeyPath);
		...
		await worker.network(networkConfig);

		// Get worker contract - information about what is connected to the worker CHECK: (is this still needed)
		context.workerContract = await this.worker.getContract()
