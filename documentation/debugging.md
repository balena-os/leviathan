...
1. To check availability of a autokit manually, you can run the following command:

```
$ curl -X POST PUBLIC-DEVICE-URL-OF-AUTOKIT/status
```

2. If the status is IDLE, then we can run a short diagnostics test to see if the autokit is working correctly. Run the Leviathan e2e test described in this section {@page Getting Started with Autokit Worker  | Start your first test run} on the autokit you like to verify. After completeing the Leviathan pre-requistes listed above the section, you can create your config.js and add the Public Device URL in the `workers` section.  
