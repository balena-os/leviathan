...
const self = (module.exports = {
  cloneRepo: async options => {
    await fse.remove(options.path);
    await git().clone(options.url, options.path);
  },

  addAndCommit: async (options, message) => {
    await git(options.path).addConfig('user.name', 'Leviathan');
    await git(options.path).addConfig('user.email', 'levithan@balena.io');
    await git(options.path).add('./*');
    await git(options.path).commit(message);
  },
  ...
  pushWaitRepoToBalenaDevice: async options => {
  ...
  clonePushWaitRepoToBalenaDevice: async options => {
    await self.cloneRepo({
      path: options.path,
      url: options.url
    });
