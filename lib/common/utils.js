...
const getSSHClientDisposer = (config) => {
  const createSSHClient = (conf) => {
    return Bluebird.resolve((new SSH()).connect(_.extend({
...
const pushRepo = async (options) => {
  ...
  await git(options.repo.path).env({
...
exports.createSSHKey = (keyPath) => {
  return fs.accessAsync(path.dirname(keyPath))
    .then(() => {
      return keygen({
        location: keyPath
      }).then(async (keys) => {
