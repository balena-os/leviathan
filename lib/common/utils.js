...
const self = (module.exports = {
  ...
  addAndCommit: async (path, message) => {
    await git(path).addConfig('user.name', 'Leviathan');
    await git(path).addConfig('user.email', 'levithan@balena.io');
    await git(path).add('./*');
  ...
  printRunQueueSummary: objTests => {
    ...
    objTests.forEach(({ _setup, tests }, i) => {
      console.log(`Setup ${i + 1}:`);
      tests.forEach((test, j) => {
        console.log(`\t${j + 1}: ${test.title}`);
      });
    });
  }
});
