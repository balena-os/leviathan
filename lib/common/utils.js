...
module.exports = {
  requireComponent: (name, instance) => {
  ...
  searchAndReplace: async (filePath, regex, replace) => {
    const content = await fs.readFileAsync(filePath, 'utf-8');
    return fs.writeFileAsync(filePath, replace(content, (regex, replace), 'utf-8'));
  },

  createSSHKey: keyPath => {
    return fs
      .accessAsync(path.dirname(keyPath))
      .then(async () => {
