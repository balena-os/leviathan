...
exports.getWorker = (options) => {
  const WORKER_DIRECTORY = './workers'

  if (options.interactiveTests === true) {
    return require(`${WORKER_DIRECTORY}/manual`)
  }

  if (options.deviceType === 'qemux86-64') {
    return require(`${WORKER_DIRECTORY}/qemu`)
  }

  return require(`${WORKER_DIRECTORY}/testbot`)
}
...
exports.waitProgressCompletion = async (promise, terminate, _times = 0, _delay = 30000) => {
  if (_times > 20) {
  ...
  if (_.chain(_.zip(initial, step))
    ...
    return exports.waitProgressCompletion(promise, terminate, _times + 1)
  }

  return exports.waitProgressCompletion(promise, terminate)
}
