...
exports.pushKernelModuleRepoToBalenaDevice = async (options) => {
  ...
  await utils.searchAndReplace(
    ...
    'files.balena-cloud.com',
    'files.balena-staging.com'
  )

  await utils.searchAndReplace(
    path.join(options.path, 'Dockerfile.template'),
    '^ENV VERSION.*$',
    `ENV VERSION ${options.balenaOSVersion}`
  )
...
exports.pushRepoToAppAndDoNotInstallIt = async (options) => {
  await cloneRepo({
    path: options.path,
    url: options.url
  })
  ...
  await waitProgressCompletion(async () => {
    ...
    const services = await options.balena.sdk.getAllServicesProperties(options.uuid, [ 'status', 'commit' ])
    const servicesFiltered = _.filter(services, _.matches({
      status: 'Downloaded',
      commit: hash
    }))

    if (_.isEmpty(servicesFiltered)) {
      return false
    }

    return _.every(servicesFiltered, {
      status: 'Downloaded',
      commit: hash
    })
  })
...
exports.searchAndReplace = async (filePath, regex, replace) => {
  ...
  shell(`sed -i 's!${regex}!${replace}!' ${filePath}`)
}
