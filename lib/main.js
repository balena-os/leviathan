...
const options = {
  ...
  proxySocks:process.env.RESINOS_TESTS_PROXY_SOCKS,
...
ava.test.before(async () => {
  ...
  if (options.proxySocks) {
    console.log('Forwarding SOCKS proxy')
    await resinio.sshHostOS('mkdir -p /mnt/boot/system-proxy/',
      context.uuid,
      context.key.privateKeyPath
    )
    console.log('this was that')
    // await resinio.sshHostOS('cat <<\'EOF\'>> /mnt/boot/system-proxy/redsocks.conf\n' +
    //   'base {\n' +
    //   'log_debug = off;\n' +
    //   'log_info = on;\n' +
    //   'log = stderr;\n' +
...
ava.test.skip('Device has started application', async (test) => {
...
ava.test.skip(`Resin host OS update [${options.resinOSVersion} -> ${options.resinOSVersionUpdate}]`, async (test) => {
