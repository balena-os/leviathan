...
if (options.interactiveTests) {
  ...
  ava.test('ResinOS image: Check if "os-release" file is available', async (test) => {
    return utils.runManualTestCase(test, {
      prepare: [ 'Download a new image from the dashboard' ],
      do: [
        'Set up a loop device by running: "losetup --show --find --partscan resin.img"',
        'Create a two temporary directory by running: "mkdir /tmp/resin-boot /tmp/resin-root"',
        'Mount the filesystem by running: "mount /dev/loop***p1 /tmp/resin-boot", replacing with the appropriate loop device',
        'You will find "os-release" file in "/tmp/resin-boot/" directory',
        'Mount the filesystem by running: "mount /dev/loop***p2 /tmp/resin-root", replacing with the appropriate loop device',
        'You will find "os-release" file in "/tmp/resin-root/balena/aufs/diff/XXXX/etc/" directory'
      ],
      assert: [
        'File "os-release" exists in root directory of the boot partition',
        'It has the same content as the one in the root partition (/etc/os-release)'
      ],
      cleanup: [
        'Unmount "/tmp/resin-boot" and "/tmp/resin-root" directory',
        'Remove "/tmp/resin-boot" and "/tmp/resin-root" directory',
