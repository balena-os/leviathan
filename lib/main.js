...
if (options.interactiveTests) {
  ...
  ava.test('supervisor: Check memory controller cgroup is enabled', async (test) => {
    return utils.runManualTestCase(test, {
      prepare: [ 'Logging into the device (ssh to the host OS)' ],
      do: [ 'Run "cat /proc/cgroups" inside the opened shell' ],
      assert: [ 'Make sure that memory controller is available and enabled by checking the last number is "1"' ],
