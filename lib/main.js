...
const main = async (suitePath) => {
  ...
  const suite = new Suite(suitePath, process.env)

  await Bluebird.using(suite.disposer(tap), (context) => {
    _.each(suite.tests, (testCase) => {
      if (testCase.interactive && !suite.options.interactiveTests) {
        return
      }

      if (testCase.deviceType) {
        const ajv = new AJV()
        if (!ajv.compile(testCase.deviceType)(context.deviceType)) {
          return
        }
      }
      ...
      tap.test(_.template(testCase.title)({
        ...
        return Reflect.apply(testCase.run, test, [ context ])
          .catch(async (error) => {
            test.threw(error)
