...
const options = {
  ...
  resinUrl: process.env.RESINOS_TESTS_RESINIO_URL,
  resinStagingUrl: process.env.RESINOS_TESTS_RESINIO_STAGING_URL,
  ...
  resultsFilePath: process.env.RESINOS_TESTS_RESULTS_PATH,
  configuration: {
...
const results = {
  author: options.email,
  deviceType: options.resinDeviceType,
  provisionTime: null,
  imageSize: null,
...
ava.test.before(async () => {
  ...
  console.log('Gather metrics')
  results.imageSize = `${fs.statSync(imagePath).size / 1048576.0} Mb`
  results.provisionTime = `${Math.floor(uptime / 60)}m ${uptime % 60}s`
...
ava.test.after.always(async () => {
  fse.outputJsonSync(options.resultsFilePath, results)
})

ava.test('Checking filename format of the image', async (test) => {
  const supervisorVersion = await resinio.getSupervisorVersion(context.uuid)
  const hostOsVersion = (await resinio.getDeviceHostOSVersion(context.uuid)).split(' ')
  const apiUrl = (await resinio.getApiUrl()).split('.')
...
ava.test(`Resin host OS update [${options.resinOSVersion} -> ${options.resinOSVersionUpdate}]`, async (test) => {
...
if (options.interactiveTests) {
  ...
  ava.test('supervisor: Reload supervisor on a running device', async (test) => {
    return utils.runManualTestCase(test, {
      ...
      do: [
        ...
        `Remove all supervisor images: "balena rmi -f $(balena images -q resin/${deviceTypeContract.arch}-supervisor)"`,
  ...
  ava.test.skip('ResinOS image: Check host OS fingerprint file on resin-root partition', async (test) => {
    return utils.runManualTestCase(test, {
      prepare: [ 'Download a new image from the dashboard and do not provision it' ],
      do: [
        'Set up a loop device by running: "losetup --show --find --partscan resin.img"',
        'Create a temporary directory by running: "mkdir /tmp/resin-root"',
        'Mount the filesystem by running: "mount /dev/loop***/p2 /tmp/resin-root", replacing with the appropriate loop device',
        'You will find "resinos.fingerprint" file in "/tmp/resin-root/balena/aufs/diff/XXXX" directory',
        'Run "md5sum" command to every file from "resinos.fingerprint"',
        'You will find "resinos.fingerprint" file in "/tmp/resin-root/balena/aufs/diff/XXXX/resin-boot" directory',
        'Run "md5sum" command to every file from "/tmp/resin-root/balena/aufs/diff/XXXX/resin-boot/resinos.fingerprint"'
      ],
      assert: [
        'The MD5 message should match the checksum from "resinos.fingerprint"',
        'This file "/tmp/resin-root/balena/aufs/diff/XXXX/resin-boot/resinos.fingerprint" should match with' +
        'the "resinos.fingerprint" file from the boot partition'
      ],
      cleanup: [
        'Unmount "/tmp/resin-root" directory',
        'Remove "/tmp/resin-root" directory',
        'Detach the loop device by running: "losetup -d /dev/loop***", replacing with the appropriate loop device'
      ]
    })
  })
}
