...
if (options.interactiveTests) {
  ...
  ava.test('supervisor: Test multicontainer application on a running device', async (test) => {
    return utils.runManualTestCase(test, {
      do: [ 'Clone https://github.com/resin-io-projects/multicontainer-getting-started inside container and push it to application' ],
      assert: [
        ...
        'You should be able to ssh using the webterminal on each of the 3 service containers'
      ]
    })
  })
