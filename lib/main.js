...
const options = {
  ...
  applicationName: `${process.env.RESINOS_TESTS_APPLICATION_NAME_PREFIX}_${testID}`,
...
ava.test.before(async () => {
  ...
  options.resinOSVersion = await utils.resolveVersionSelector(
    await resinio.getAllSupportedOSVersions(options.deviceType),
    options.resinOSVersion)
