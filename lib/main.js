...
if (options.interactiveTests) {
  ...
  ava.test.skip('ResinOS image: Check host OS fingerprint file on resin-boot partition', async (test) => {
    return utils.runManualTestCase(test, {
      ...
      do: [
        ...
        'Create a temporary directory by running: "mkdir /tmp/resin-boot"',
        'Mount the filesystem by running: "mount /dev/loop*** /tmp/resin-boot", replacing with the appropriate loop device',
        'You will find "resinos.fingerprint" file in "/tmp/resin-boot" directory',
        'Run "md5sum" command to every file from "resinos.fingerprint"'
      ],
      assert: [ 'The MD5 message should match the checksum from "resinos.fingerprint"' ],
      cleanup: [
        'Unmount "/tmp/resin-boot" directory',
        'Remove "/tmp/resin-boot" directory',
