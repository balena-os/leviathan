...
const setup = async () => {
  ...
  await balena.loginWithToken(options.apiKey)

  console.log(await balena.whoami())
  console.log('is logged in: ', await balena.isLoggedIn())

  console.log(`Creating application: ${options.applicationName} with device type ${options.deviceType}`)
  console.log(await balena.createApplication(options.applicationName, options.deviceType))

  context.key = await balena.createSSHKey(options.sshKeyLabel)
  ...
  if (options.delta) {
    ...
    await balena.setAppConfigVariable(options.applicationName, 'RESIN_SUPERVISOR_DELTA', options.delta)
  }

  console.log(`Creating device placeholder on ${options.applicationName}`)
  const placeholder = await balena.createDevicePlaceholder(options.applicationName)

  console.log(`Getting configuration for device ${placeholder.uuid}`)
  const balenaConfiguration = await balena.getDeviceOSConfiguration(
  ...
  await utils.waitUntil(() => {
    return balena.isDeviceOnline(placeholder.uuid)
  })
  context.uuid = placeholder.uuid

  const uptime = await utils.getDeviceUptime((command) => {
    return balena.sshHostOS(command, context.uuid, context.key.privateKeyPath)
  })
  ...
  results.email = await balena.getEmail()

  console.log('Running tests:')
  context.dashboardUrl = await balena.getDashboardUrl(context.uuid)
}
