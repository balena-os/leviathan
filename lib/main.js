...
const options = {
  ...
  metricsPath: process.env.RESINOS_TESTS_METRICS_PATH,
  configuration: {
    network: 'ethernet'
  }
}
...
ava.test.before(async () => {
  ...
  metrics.imageSize = `${fs.statSync(imagePath).size / 1000000.0} Mb`
  ...
  await utils.waitUntil(async () => {
    return (await resinio.getDeviceHostOSVersion(context.uuid)) !== 'null'
  })
})

ava.test.after.always(async (test) => {
  fse.outputJsonSync(options.metricsPath, metrics)
})

ava.test('Device is online', async (test) => {
  const isOnline = await resinio.isDeviceOnline(context.uuid)

  const uptime = await utils.getUptime((command) => {
    return resinio.sshHostOS(command, context.uuid, context.key.privateKeyPath)
  })

  metrics.provisionTime = `${Math.floor(uptime / 60)}m ${uptime % 60}s`
  test.true(isOnline)
})
