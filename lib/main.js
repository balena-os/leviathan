...
const options = {
  ...
  apiUrl: process.env.RESINOS_TESTS_API_URL,
  downloadApiUrl: process.env.RESINOS_TESTS_DOWNLOAD_API_URL,
  configuration: {
    network: 'ethernet'
  }
}
...
ava.test.before(async () => {
  ...
  await resinio(options.apiUrl).loginWithCredentials({
    email: options.email,
    password: options.password
  })

  console.log(`Creating application: ${options.applicationName} with device type ${options.deviceType}`)
  await resinio(options.apiUrl).createApplication(options.applicationName, options.deviceType)

  console.log('Remove previous SSH keys')
  await resinio(options.apiUrl).removeSSHKeys()
  context.key = await resinio(options.apiUrl).createSSHKey()
  console.log(`Add new SSH key: ${context.key.publicKey}`)

  console.log(`Downloading device type OS into ${imagePath}`)
  if (!_.isUndefined(options.downloadApiUrl)) {
    await resinio(options.downloadApiUrl).downloadDeviceTypeOS(options.deviceType, options.resinOSVersion, imagePath)
  } else {
    await resinio(options.apiUrl).downloadDeviceTypeOS(options.deviceType, options.resinOSVersion, imagePath)
  }

  console.log(`Creating device placeholder on ${options.applicationName}`)
  const placeholder = await resinio(options.apiUrl).createDevicePlaceholder(options.applicationName)

  console.log(`Getting resin.io configuration for device ${placeholder.uuid}`)
  const resinConfiguration = await resinio(options.apiUrl).getDeviceOSConfiguration(
  ...
  await utils.waitUntil(async () => {
    return resinio(options.apiUrl).isDeviceOnline(placeholder.uuid)
  })
...
ava.test.after.always(async () => {
  ...
  await resinio(options.apiUrl).removeApplication(options.applicationName)
})

ava.test('device should become online', async (test) => {
  const isOnline = await resinio(options.apiUrl).isDeviceOnline(context.uuid)
  test.true(isOnline)
})

ava.test.skip('device should report hostOS version', async (test) => {
  const version = await resinio(options.apiUrl).getDeviceHostOSVersion(context.uuid)
...
ava.test('should push an application', async (test) => {
  ...
  const gitUrl = await resinio(options.apiUrl).getApplicationGitRemote(options.applicationName)
  ...
  await utils.waitUntil(async () => {
    return await resinio(options.apiUrl).getDeviceStatus(context.uuid) === 'Downloading'
  })
  await utils.waitUntil(async () => {
    return await resinio(options.apiUrl).getDeviceStatus(context.uuid) === 'Idle'
  })

  const commit = await resinio(options.apiUrl).getDeviceCommit(context.uuid)
  ...
  ava.test('supervisor: reload supervisor on a running device', async (test) => {
    return utils.runManualTestCase(test, {
      ...
      do: [
        ...
        'Remove all supervisor images by running, for example, `balena rmi -f $(balena images -q resin/{ARCH}-supervisor)`',
