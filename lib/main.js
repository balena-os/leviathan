...
ava.test.before(async () => {
  ...
  await resinos.injectNetworkConfiguration(options.configuration, (contents, destination) => {
...
if (options.configuration.network !== 'cellular' && options.modemAPN && options.modemNumber) {
  ava.test('Cellular modem connection', async (test) => {
    context.uuid = 'a4f530d820ae91a83a04524bf7ef2ec4'
    context.key = {}
    context.key.privateKeyPath = '/usr/app/id_rsa'
    await resinos.injectNetworkConfiguration({
      network: 'cellular',
      apn: options.modemAPN,
      number: options.modemNumber,
      autoconnect: 'no'
    }, (contents, destination) => {
      const mountpoint = `findmnt --canonicalize --noheadings --output SOURCE /mnt/sysroot/active | \
        sed -s "s/\\(.\\)$/${destination.partition}/" | \
        xargs findmnt --canonicalize --noheadings --output TARGET \
      `
      return resinio.sshHostOS(`echo "${contents}" > "$(${mountpoint})${destination.path}"`,
        context.uuid, context.key.privateKeyPath)
    })

    await resinio.sshHostOS('systemctl restart resin-net-config', context.uuid, context.key.privateKeyPath)
    await resinio.sshHostOS('nmcli c reload', context.uuid, context.key.privateKeyPath)
    await resinio.sshHostOS('nmcli c up resin-gsm', context.uuid, context.key.privateKeyPath)

    const iface = await resinio.sshHostOS(
      'ip -o addr | grep $(nmcli c show resin-gsm | grep "IP4.ADDRESS" | awk \'{print $2}\') | awk \'{print $2}\'',
      context.uuid, context.key.privateKeyPath
    )

    test.is(await resinio.sshHostOS(`curl -s -I -L --interface ${iface} https://www.google.com  | grep HTTP | awk '{print $2}'`, context.uuid, context.key.privateKeyPath), '200')

    await resinio.reboot(context.uuid)
