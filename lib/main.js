...
if (options.interactiveTests) {
  ...
  ava.test('supervisor: Checking that the device does not return the resin-vpn IP address', async (test) => {
    return utils.runManualTestCase(test, {
      prepare: [ 'Logging into the device (ssh to the host OS)' ],
      do: [
        'Run "ifconfig" inside the opened shell of host OS',
        `Run "resin device ${context.uuid}" `
      ],
      assert: [ 'Check the "resin-vpn" IP address from the command "ifconfig". This IP shouldn\'t be returned by resin-cli' ],
