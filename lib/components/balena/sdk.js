...
module.exports = class BalenaSDK {
  ...
  async sshHostOS (command, uuid, privateKeyPath) {
    ...
    const options = [ '-p 22', `${await this.balena.auth.whoami()}@ssh.${await this.balena.settings.get('proxyUrl')}`,
      `host -s ${uuid}` ]
  ...
  async sshUserContainer (command, uuid, privateKeyPath) {
    const containerId = await this.balena.models.device.getApplicationInfo(uuid).get('containerId')

    if (_.isEmpty(containerId)) {
      throw new Error('No user container found running')
    }
  ...
  getDeviceName (device) {
    return this.balena.models.device.get(device).get('device_name')
  }
