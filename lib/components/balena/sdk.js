...
module.exports = class BalenaSDK {
  ...
  async sshHostOS (command, uuid, privateKeyPath) {
    ...
    console.log('\n====  BEFORE  ====')
    console.log('is_online:', await this.isDeviceOnline(uuid))
    console.log('getDeviceStatus:', await this.getDeviceStatus(uuid))
    console.log('is_connected_to_vpn: ', await this.balena.models.device.get(uuid).get('is_connected_to_vpn'))
    console.log('last online: ', await this.balena.models.device.lastOnline(uuid))
    console.log('status: ', await this.balena.models.device.get(uuid).get('status'))
    console.log('is_active: ', await this.balena.models.device.get(uuid).get('is_active'))

    await utils.waitUntil(async () => {
      console.log('Check if device is online: ',
        await this.isDeviceOnline(uuid) && semver.gte(await this.getDeviceHostOSVersion(uuid), VERSION_REQUIREMENT))
