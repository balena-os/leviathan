...
module.exports = class BalenaSDK {
  ...
  async sshHostOS (command, uuid, privateKeyPath) {
    ...
    await utils.waitUntil(async () => {
      return await this.isDeviceConnectedToVpn(uuid) && semver.gte(await this.getDeviceHostOSVersion(uuid), VERSION_REQUIREMENT)
    })

    return utils.ssh(`host -s ${uuid} source /etc/profile ; ${command}`, {
