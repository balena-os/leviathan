...
const waitProgressCompletion = async (promise, terminate, _times = 20, _delay = 30000) => {
  if (_times === 0) {
  ...
  if (
    flow([
      ...
      every(pair => {
        return isNumber(pair[1]) && isNumber(pair[0]) && pair[1] - pair[0] === 0;
      })
    ])(initial, step)
  ) {
    return waitProgressCompletion(promise, terminate, _times - 1, _delay);
  }

  return waitProgressCompletion(promise, terminate, _times, _delay);
};
