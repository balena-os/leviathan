...
module.exports = class BalenaOS {
  ...
  get unpack () {
    return {
      jenkins: async () => {
        const supervisorVersion = await fs.readFileAsync(path.join(this.download.source, 'VERSION'))
        ...
        return {
          ...
          version,
          stream: fs.createReadStream(path.join(this.download.source, 'resin.img.zip')).pipe(unzip.Parse()),
      ...
      imageMaker: async () => {
        ...
        const repo = RegExp('.*-staging\..*').test(this.download.source) ? 'resin/resinos-staging' : 'resin/resinos'

        return {
          hostapp: {
            type: 'remote',
            source: `${repo}:${(await utils.resolveVersion(supportedVersions)).replace('+', '_').replace(/\.(prod|dev)$/, '')}-${this.deviceType}`
          },
          version,
          filename,
          stream
  ...
  hostappCommand () {
    return `hostapp-update -r ${this.hostapp.type === 'remote' ? '-i' : '-f'} ${this.hostapp.source}`
  }
  ...
  async fetch (destination) {
    ...
    const {
      ...
      filename,
      stream
    } = await this.unpack[this.download.type]()
