...
const { markDownFormat } = require('./format');
...
const createNewFragment = async (results) => {
	...
	const newFragment = {
		device_type: results.balenaDeviceType,
		provision_time: results.provisionTime,
		image_size: results.imageSize,
		balena_os_version: results.balenaOSVersion,
		body: results.body,
	...
	if (diff.length > 0) {
		throw new Error(
			`ERROR: Some fragment property names do not exist in the pensieve schema ${diff}`,
		);
	}
...
module.exports = async (output, DATA_STORE_PATH, DATA_STORE) => {
	...
	console.log(
		`Publising test results to ${pensieve.backend.account}.github.io/${pensieve.backend.name}`,
	);
