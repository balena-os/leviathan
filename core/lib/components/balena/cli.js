	...
	preload(image, options) {
		return new Promise(async (resolve, reject) => {
			...
			const Container = docker.getContainer(
				...
				(await exec(
					'cat /proc/self/cgroup | head -1 | sed "s/.*docker\\///g" | tr -d "\n"',
				))[0],
			);
			...
			const child = spawn(
				'balena',
				[
					`preload --docker /var/run/docker.sock --app ${
						options.app
					} --commit ${options.commit} --pin-device-to-release ${
						options.pin
					} ${join(Mount.Source, basename(image))}`,
				],
				{
					stdio: 'inherit',
					shell: true,
